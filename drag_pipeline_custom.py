...
from utils.ldm.modules.diffusionmodules.openaimodel import MultiViewUNetModel
import torch
import torch.nn.functional as F
import inspect
import numpy as np
...
from utils.mv_unet import MultiViewUNetModel, get_camera
...
class MVDreamPipeline(DiffusionPipeline):
    ...
    def forward_unet_features(self, z, t, encoder_hidden_states, layer_idx=[0], interp_res_h=256, interp_res_w=256):
        ...
        # print("input of unet:")
        # print(z.shape)
        # print(torch.tensor([t] * 4 , dtype=z.dtype, device='cuda').shape)
        # print(encoder_hidden_states.shape)
        # print(self.camera.shape)
        unet_inputs = {
            'x': z,
            'timesteps': torch.tensor([t] * 4 , dtype=z.dtype, device='cuda'),
            'context': encoder_hidden_states[:4],
            'num_frames': 4,
            'camera': self.camera,
            'return_intermediates': True
        }
        unet_output, all_intermediate_features = self.unet.forward(**unet_inputs)
    ...
    def invert(
        ...
        **kwds):
        ...
        This function is modified on dragdiffusion.
        """
        DEVICE = torch.device("cuda") if torch.cuda.is_available() else torch.device("cpu")
        ...
        latents = self.encode_image_latents(image, device=DEVICE, num_images_per_prompt=4)[1]

...
class AttentionBase:
    ...
    def __call__(self, q, k, v, is_cross, place_in_unet, num_heads, attention_op, **kwargs):
        out = self.forward(q, k, v, is_cross, place_in_unet, num_heads, attention_op, **kwargs)
    ...
    def forward(self, q, k, v, is_cross, place_in_unet, num_heads, attention_op, **kwargs):
        dim_head = 64
        b, _, _ = q.shape
        q, k, v = map(
            lambda t: t.unsqueeze(3)
            .reshape(b, t.shape[1], num_heads, dim_head)
            .permute(0, 2, 1, 3)
            .reshape(b * num_heads, t.shape[1], dim_head)
            .contiguous(),
            (q, k, v),
        )
        ...
        out = (
            out.unsqueeze(0)
            .reshape(b, num_heads, out.shape[1], dim_head)
            .permute(0, 2, 1, 3)
            .reshape(b, out.shape[1], num_heads * dim_head)
        )
        return out
...
class MutualSelfAttentionControl(AttentionBase):
    ...
    def forward(self, q, k, v, is_cross, place_in_unet, num_heads, attention_op, **kwargs):
        ...
        if self.guidance_scale > 1.0:
            ...
            # [2, 5, channels, dim]
            q = torch.cat([q[0:1], q[1:2]], dim=2).squeeze(0)
            #  [5, channels*2, dim]
            # print("q.shape 2:",q.shape)
            out = xformers.ops.memory_efficient_attention(
                q, k[0:1].squeeze(0), v[0:1].squeeze(0), attn_bias=None, op=attention_op
            )
            # print("out.shape 0:",out.shape)
            out = torch.cat(out.chunk(2, dim=1), dim=0) # split the queries into source and target batch
...
def override_mvdream_attn_forward(attn, editor, place_in_unet):
    def forward(x, context=None):
        ...
        #     context_ip = context[:, -attn.ip_dim :, :]
        #     k_ip = attn.to_k_ip(context_ip)
        #     v_ip = attn.to_v_ip(context_ip)
        #     context = context[:, : (token_len - attn.ip_dim), :]

        k = attn.to_k(context)
        v = attn.to_v(context)
        # actually compute the attention, what we cannot get enough of
        # out = xformers.ops.memory_efficient_attention(
        #     q, k, v, attn_bias=None, op=attn.attention_op
        # )
        out = editor(
            q, k, v, is_cross, place_in_unet,

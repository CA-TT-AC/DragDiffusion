...
class MultiViewUNetModel(ModelMixin, ConfigMixin):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # to save all_intermediate_features
        all_intermediate_features = [h]
        
        for module in self.output_blocks:
            h = torch.cat([h, hs.pop()], dim=1)
            h = module(h, emb, context, num_frames=num_frames)
            all_intermediate_features.append(h)
        h = h.type(x.dtype)
        # is false defaultly
        if self.predict_codebook_ids:
            ...
            if return_intermediates:
                idxs = [0, 3, 6, 9, 12]
                key_intermediate_features = [all_intermediate_features[i] for i in idxs]
                return self.out(h), key_intermediate_features
            return self.out(h)